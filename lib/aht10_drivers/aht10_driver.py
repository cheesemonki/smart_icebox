...
class AHT10:
    ...
    time.sleep_ms(450)
    calibration_status= self.i2c.readfrom(self.address, 1)  # 读取状态字节
    print("AHT10 calibration status:", calibration_status)  # 打印状态字节
...
def read_humidity(self):
    ...
    if self.is_calibration_enabled():
        ...
        while temp_hex[2] == 0:
            print("Data reading failed. Trying again...")
            temp = self.i2c.readfrom(self.address, 6)
            temp_hex = struct.unpack(">BBBBBB", temp)
